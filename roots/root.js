...
const { getHiddenPosts, removeHiddenPost } = require('../controllers/userControllers/HiddenPostcontroller');
const { getAllJobs } = require('../controllers/JobController/jobpostController');
const { getAllJobsforMobile } = require('../controllers/JobController/jobcontroller');
...
router.post("/user/profile/cover/update", auth, userUpload.single("coverPhoto"), (req, res, next) => {
  req.baseUrl = "/profile/cover"; next();
},
  userUploadToCloudinary,
  updateCoverPhoto
);
...
router.get("/get/user/jobs", auth, getAllJobsforMobile);
