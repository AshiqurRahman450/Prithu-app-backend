...
exports.getAllJobs = async (req, res) => {
  try {
    // 1️⃣ Get jobs with company login details
    const jobs = await JobPost.find({})
      .sort({ createdAt: -1 })
      .populate({
        path: "postedBy", // Company Login
        select: "companyName email",
      })
      .lean();

    // 2️⃣ Extract company IDs
    const companyIds = jobs
      .map(job => job.postedBy?._id)
      .filter(Boolean);

    // 3️⃣ Fetch company profiles in ONE query
    const companyProfiles = await CompanyProfile.find(
      { companyId: { $in: companyIds } },
      "companyId logo website location industry"
    ).lean();

    // 4️⃣ Create lookup map
    const companyProfileMap = {};
    companyProfiles.forEach(profile => {
      companyProfileMap[profile.companyId.toString()] = profile;
    });

    // 5️⃣ Merge job + company login + company profile
    const jobsWithCompanyDetails = jobs.map(job => {
      const companyProfile =
        companyProfileMap[job.postedBy?._id?.toString()] || {};
    ...
    console.log("wwww",jobs)
    res.status(200).json({
      success: true,
      total: jobsWithCompanyDetails.length,
