...
exports.userVideoViewCount = async (req, res) => {
  try {
    ...
    // 4️⃣ Push view + increment user's total duration (use feed.duration)
    await VideoView.findOneAndUpdate(
      { userId },
      {
        $push: {
          views: {
            videoId: feedId,
            watchedSeconds: feed.duration || 0, // ✅ use feed duration
            totalDuration: feed.duration || 0, // ✅ store feed's duration
            viewedAt: new Date(),
          },
        },
        $inc: { totalDuration: feed.duration || 0 }, // ✅ increment total by feed duration
      },
      { upsert: true }
    );
