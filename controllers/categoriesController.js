...
exports.getfeedWithCategoryWithId = async (req, res) => {
  try {
    ...
    // 3️⃣ Enrich feeds with cached themes (same as All feeds!)
    const DEFAULT_THEME = {
      primary: "#4A90E2",
      secondary: "#7B68EE",
      accent: "#50C878",
    ...
    // Initialize theme cache if not exists (shared with All feeds)
    ...
    const enrichedFeeds = feeds.map((feed) => {
      ...
      return {
        ...feed,
        framedAvatar: null,
        themeColor: cachedTheme || feed.themeColor || DEFAULT_THEME,
        timeAgo: feedTimeCalculator(feed.createdAt),
      };
    });

    // 🔥 Extract themes in background (non-blocking) - same as All feeds
    setImmediate(async () => {
...
exports.getUserContentCategories = async (req, res) => {
  try {
    ...
    // Include feeds with null/undefined language OR matching user's language
    const feedMatch = feedLang
      ? { $or: [{ language: feedLang }, { language: null }, { language: { $exists: false } }] }
      : {};

    // 3️⃣ Find all unique feed category IDs in user's language
    console.log("Feed match condition:", JSON.stringify(feedMatch));
